...
import { Search, UserCircle, Github, Linkedin, Twitter } from "lucide-react";
...
function MemberCard({ member, className }: { member: typeof allMembers[0], className?: string }) {
  const img = PlaceHolderImages.find(p => p.id === member.imgId);
  return (
    <Card className={cn("overflow-hidden border-none shadow-lg bg-white group hover:shadow-2xl transition-all duration-300 rounded-2xl", className)}>
      <CardContent className="p-0 flex flex-col">
        <div className="relative aspect-square bg-muted overflow-hidden">
          ...
          <div className="absolute inset-0 bg-primary/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
            <button className="h-10 w-10 rounded-full bg-white flex items-center justify-center text-primary hover:bg-accent hover:text-white transition-colors shadow-sm">
        ...
        <div className="p-6 text-center space-y-2">
          <h3 className="font-bold text-primary text-lg md:text-xl line-clamp-1">{member.name}</h3>
          <p className="text-xs text-accent font-bold uppercase tracking-widest">{member.role}</p>
        </div>
      </CardContent>
    </Card>
  );
    ...
    <div className="min-h-screen bg-background py-16">
      <div className="container mx-auto px-4 space-y-16">
        ...
        <div className="max-w-xl mx-auto space-y-6 text-center">
          <h1 className="text-4xl md:text-5xl font-headline font-bold text-primary">Meet the Visionaries</h1>
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-muted-foreground" />
            <Input 
              placeholder="Search team members by name or role..." 
              className="pl-12 h-14 rounded-2xl border-primary/20 bg-white shadow-sm focus-visible:ring-primary/30 text-lg"
              value={searchQuery}
        ...
        {filteredMembers.length > 0 ? (
          <div className="space-y-20">
            {/* Tier 1: Chief / Founder */}
            {tier1.length > 0 && (
              <div className="flex flex-col items-center space-y-6">
                <div className="flex justify-center">
                  {tier1.map((member, i) => (
                    <MemberCard key={i} member={member} className="w-72" />
                  ))}
                </div>
              </div>
            )}

            {/* Tier 2: Advisors & Mentors */}
            {tier2.length > 0 && (
              <div className="space-y-8">
                <div className="flex flex-wrap justify-center gap-8 md:gap-12">
                  {tier2.map((member, i) => (
                    <MemberCard key={i} member={member} className="w-64" />
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="relative py-4">
              ...
              <div className="relative flex justify-center">
                <span className="bg-background px-4 text-sm font-semibold text-muted-foreground uppercase tracking-widest">Executive Council</span>
              </div>
            </div>

            {/* Tier 3: General Executives Grid */}
            {tier3.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8 md:gap-10 max-w-6xl mx-auto">
                {tier3.map((member, i) => (
                  <MemberCard key={i} member={member} className="w-full" />
                ))}
              </div>
            )}
          </div>
